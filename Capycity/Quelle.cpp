...
public:
    ...
    vector<Material> getBenoetigtesMaterial() {
        return benoetigteMaterialien;
    }

    void printMaterialien() {
        cout << "[";
        for (Material m : benoetigteMaterialien)
            cout << m.getName() << ", ";
        cout << "]";
    }
};
    ...
    void print_baubereich() {                                           //Ausgabe des Bauplans
        for (int i = 0; i < laengeBaubereich; i++)
        {
            for (int j = 0; j < breiteBaubereich; j++)
            {
                ...
                for (Building b : vorhandeneGebaeude) {                 //Prüfen, ob Gebäude schon gelistet
                    if (b.getLabel() == baubereich[j][i].getLabel() || baubereich[j][i].getLabel() == "leer")
                        buildingFound = true;
                }

                if (!buildingFound)                                     //Gebäude wird eingetragen, falls noch nicht getan
        ...
        for (Building b : vorhandeneGebaeude) {                             //Ausgabe der Gebäude und ihren Materialien und Preisen + Gesamtpreis
            cout << endl << b.getLabel() << "\t" << b.getGroesse() << "x ";
            b.printMaterialien();
            cout << "\t" << b.getEinzelpreis();
            gesamtPreis += b.getEinzelpreis();
        }

        cout << endl << "Gesamtpreis: " << gesamtPreis << endl << endl;
        gesamtPreis = 0;
